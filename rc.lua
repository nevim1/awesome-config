...
--[[ if you want to have the same colors as firefox
beautiful.border_focus = '#545458'
...
local mymainmenu = awful.menu(
	{ items = {
		...
		{ "pick color to clipboard", function() awful.spawn("xcolor -s clipboard") end },
		{ "Prinscreen", function()
			awful.spawn.with_shell("xwd -root | magick XWD:- ~/Pictures/Screenshots/" .. os.date('%Y-%m-%d_%H:%M:%S.png'))
			naughty.notify({
				title = "Screenshot saved",
				text = "screenshot was saved to ~/Pictures/Screenshots/" .. os.date('%Y-%m-%d_%H:%M:%S.png')
			})
		end},
...
function toHex(digits, num)
	...
	for i = digits-1,0,-1 do
		digit, frac = math.modf(number / 15^i)
		out = out .. hexLookup[digit + 1]
		number = number - digit * 15^i
	end
	return out
end
...
globalkeys = gears.table.join(
	...
	awful.key(
		{modkey, "Shift"}, "Delete", awesome.quit,
	...
	-- {{{ Screenshot keys
	--awful.key({}, "Print", function() awful.spawn("xwd -root | magick xwd: png:~/Pictures/Screenshots/testAwsome.png") end, { description = "Скриншот экрана", group = "screenshot" }),
	awful.key({}, "Print", function() awful.spawn("spectacle") end, { description = "Скриншот экрана", group = "screenshot" }),

	awful.key({ "Shift" }, "Print", function() end, { description = "Скриншот области", group = "screenshot" }),

	awful.key({ "Control" }, "Print", function() end, { description = "Скриншот окна", group = "screenshot" })
	-- }}}
)
-- }}}
...
clientkeys = gears.table.join(
	...
	awful.key(
		{modkey, "Control"}, "Return", function (c) c:swap(awful.client.getmaster())
		end,
		{description = "move to master", group = "client"}
	),

	awful.key(
		{modkey}, "o", function (c) c:move_to_screen()
		end,
		{description = "move to screen", group = "client"}
	),

	awful.key(
		{modkey}, "t", function (c) c.ontop = not c.ontop
		end,
